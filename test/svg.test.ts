...
describe("iconProblems", () => {
  ...
  it.each([
    ...
    ["a use element, even one that points inside the document", svg('<defs><path id="a" d="M0 0"/></defs><use href="#a"/>')],
    ["a use fan-out that expands to millions of shapes", svg(`<defs><path id="a0" d="M0 0"/>${Array.from({ length: 20 }, (_, i) => `<g id="a${i + 1}"><use href="#a${i}"/><use href="#a${i}"/></g>`).join("")}</defs><use href="#a20"/>`)],
    ...
    ["a harmless-looking style attribute", svg('<path d="M0 0" style="fill:red"/>')],
    ["a CSS escape that spells url(", svg(String.raw`<path d="M0 0" fill="\75rl(https://x/a.png)"/>`)],
    ["a CSS escape that spells javascript:", svg(String.raw`<path d="M0 0" fill="\6aavascript:alert(1)"/>`)],

...
export function createGitHub(token: string, doFetch: typeof fetch = fetch): GitHub {
  ...
  return {
    ...
    async getFile(repo, path, ref) {
      if (!/^[0-9a-f]{40}$/.test(ref)) throw new Error("a file is read at a full commit SHA, never at a branch name");
    ...
    async upsertComment(repo, pr, marker, body) {
      const comments = (await get<Array<{ id: number; body?: string; user?: { login?: string } }>>(`/repos/${checked(repo)}/issues/${pr}/comments?per_page=100`)) ?? [];
